    ...
    <main>
      ...
      <section className="collection" id="collection">
        ...
        {view === "references" ? <div className="reference-library">
          {!!developmentResults.length && <section className="development-archive"><button className="museum-launch" onClick={() => setTourRoom(0)}><span><small>Pocket Archives presents</small><b>The history of Pokémon design</b><em>A self-guided museum tour through 8 chronological rooms</em></span><strong>Start tour <i>→</i></strong></button><div className="timeline-heading"><span>1990–1997 · Earliest material first</span><h3>Alpha &amp; beta archive</h3><p>Original concepts, extracted prototype assets, release-era Carddass illustrations, and research plates are labeled separately.</p></div><div className="thread-source-note"><p><b>Added from the Reddit trail.</b> All 52 unique images from the four primary albums linked by the poster are now inside the gallery. The classic Red &amp; Green watercolor set was already available as alternate artwork in the Pokédex, so it has not been duplicated here.</p><span>Source record · r/gaming · 2011</span></div><div className="research-square-grid" aria-label="Local prototype research explanations">{helixResearchPanels.map((panel, index) => <button key={panel.id} onClick={() => setSelectedPanel(panel)}><small>{String(index + 1).padStart(2, "0")} · Research note</small><b>{panel.title}</b><span>Read here +</span></button>)}</div><div className="development-grid">{developmentResults.map((item) => <article className="development-card" key={item.id}><button onClick={() => setSelectedDevelopment(item)} aria-label={`Open ${item.title}`}><span className="development-image"><img src={item.src} alt={item.title} loading="lazy" /></span><span className="development-card-copy"><small>{item.year} · {item.kind}</small><strong>{item.title}</strong><em>{item.credit}</em></span><span className="sheet-open">Open historical plate +</span></button></article>)}</div></section>}

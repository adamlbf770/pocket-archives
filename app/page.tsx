    ...
    <main>
      ...
      <section className="collection" id="collection">
        ...
        {view === "references" ? <div className="reference-library" id="archive-browser">
          ...
          {museumWing === "lobby" && <><button className="museum-launch" onClick={() => setTourRoom(0)}><span><small>Recommended first visit · Guided exhibition</small><b>The history of Pokémon design</b><em>8 chronological rooms · approximately 6–8 minutes</em></span><strong>Begin here <i>→</i></strong></button><div className="museum-room-intro"><span>Continue exploring</span><h3>Choose a deeper room.</h3><p>After the tour, follow whichever part of the story interests you. Each room opens as its own focused visit.</p></div><div className="museum-room-choices"><button className="museum-room-choice periods" onClick={() => setMuseumWing("periods")}><small>Room 02 · Internal archaeology</small><strong>The 190</strong><p>See how the original roster developed across five broad design periods.</p><span>Enter room →</span></button><button className="museum-room-choice research" onClick={() => setMuseumWing("research")}><small>Room 03 · Evidence room</small><strong>Research Desk</strong><p>Learn how prototypes, internal numbers, cries, maps, and lost designs are interpreted.</p><span>Enter room →</span></button></div></>}
          {museumWing === "periods" && <section className="internal-list-wing museum-focused-wing"><button className="museum-wing-back" onClick={() => setMuseumWing("lobby")}>← Museum lobby</button><div className="museum-wing-heading"><span>Room 02 · Internal archaeology</span><h3>The 190</h3><p>Read from Period 1 through Period 5. This is Helix Chamber’s research framework for the original Red &amp; Green roster—not a confirmed production timeline.</p></div><div className="museum-stat-strip"><div><b>190</b><span>internal slots</span></div><div><b>151</b><span>released Pokémon</span></div><div><b>39</b><span>unused slots</span></div><div><b>5</b><span>design periods</span></div></div><div className="internal-period-grid">{internalListPanels.map((panel, index) => <button key={panel.id} onClick={() => setSelectedPanel(panel)}><small>Period {String(index + 1).padStart(2, "0")}</small><b>{panel.title}</b><p>{panel.description}</p><span>Read period →</span></button>)}</div></section>}

    ...
    <main>
      ...
      <section className="collection" id="collection">
        ...
        {view === "references" ? <div className="reference-library">
          ...
          {!!resourceResults.length && <aside className="resource-sidebar"><div className="reference-heading compact"><span>Artist toolkit</span><h3>More resources</h3></div><div className="resource-grid">{resourceResults.map((resource) => <button className="resource-card" onClick={() => setSelectedPanel({ id: `resource-${resource.title}`, eyebrow: resource.eyebrow, title: resource.title, description: resource.description, details: ["This reference has been summarized inside Pocket Archives so browsing never leaves the collection.", "Use the archive search to find related Pokémon sheets, poses, expressions, movement studies, and artwork.", "The original catalog record is preserved as provenance but is intentionally not presented as an outbound link."], source: "Catalogued from the PS Art Room resource index." })} key={resource.title}><span>{resource.eyebrow}</span><h3>{resource.title}</h3><p>{resource.description}</p><b>Open here +</b></button>)}</div></aside>}</div>
      ...
      {selectedReference && <div className="reference-viewer" role="dialog" aria-modal="true" aria-label={`${selectedReference.group.name} reference sheet`} onMouseDown={(event) => { if (event.currentTarget === event.target) setSelectedReference(null); }}>
        ...
        <aside className="reference-viewer-details">
          ...
          <p className="local-viewer-note">The full-resolution sheet is displayed here; no external page is opened.</p>
      ...
      {selectedDevelopment && <div className="reference-viewer development-viewer" role="dialog" aria-modal="true" aria-label={`${selectedDevelopment.title} early development plate`} onMouseDown={(event) => { if (event.currentTarget === event.target) setSelectedDevelopment(null); }}>
        ...
        <aside className="reference-viewer-details">
          ...
          <p className="local-viewer-note">Source research is summarized in the local research-note squares.</p>

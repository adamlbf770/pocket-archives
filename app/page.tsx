    ...
    <main>
      ...
      <section className="collection" id="collection">
        ...
        {view === "references" ? <div className="reference-library" id="archive-browser">
          ...
          {archiveSection === "references" && <section className="production-room"><div className="archive-provenance"><div><span>Production room</span><h3>What are these sheets?</h3></div><div><p><b>Settei</b> are working reference drawings used to keep characters, poses, expressions, and proportions consistent in animation.</p><p>Each popup now keeps only the sheet title and its source record, avoiding the same explanation on every drawing.</p><p className="provenance-record">PS Art Room index · OLM production references</p></div></div><div className="reference-heading"><span>1997–present · Pokédex order</span><h3>Production references</h3><p>{referenceItems.length} sheets</p></div>{setteiDirectory.length === 0 ? <div className="loading-grid">Indexing the reference room…</div> : referenceItems.length === 0 ? <div className="empty-state"><span>?</span><h3>No reference sheets found</h3><p>Try another Pokémon, pose, or expression.</p><button onClick={() => setQuery("")}>Clear search</button></div> : <div className="settei-gallery">{referenceItems.slice(0, visible).map(({ group, link, index }) => { const archived = link.url.includes("web.archive.org/web/"); return <article className="sheet-card" key={`${group.dex}-${link.url}-${index}`}><button onClick={() => openReference(group, index)} aria-label={`Open ${group.name} ${link.label} in the reference viewer`}><span className="sheet-image"><img src={referenceDestination(link.url)} alt={`${group.name} ${link.label}`} loading="lazy" /></span><span className="sheet-card-copy"><small>#{String(group.dex).padStart(4, "0")}{archived ? " · Preserved copy" : " · Production art"}</small><strong>{group.name}</strong><em>{link.label === "Model sheet" && index > 0 ? `Model sheet ${index + 1}` : titleCase(link.label)}</em></span><span className="sheet-open">Open sheet +</span></button></article>; })}</div>}</section>}
          {archiveSection === "resources" && (!!resourceResults.length ? <section className="artist-resources"><div className="reference-heading"><span>Drawing room</span><h3>Artist resources</h3><p>{resourceResults.length} collections</p></div><div className="resource-grid">{resourceResults.map((resource) => <button className="resource-card" onClick={() => setSelectedPanel({ id: `resource-${resource.title}`, eyebrow: resource.eyebrow, title: resource.title, description: resource.description, details: ["Use this collection as a starting point for related poses, movement studies, tools, or techniques.", "The original catalog record is preserved in Pocket Archives without sending you to another page."], source: "Catalogued from the PS Art Room resource index." })} key={resource.title}><span>{resource.eyebrow}</span><h3>{resource.title}</h3><p>{resource.description}</p><b>View summary +</b></button>)}</div></section> : <div className="empty-state"><span>?</span><h3>No resources found</h3><p>Try a broader search.</p><button onClick={() => setQuery("")}>Clear search</button></div>)}

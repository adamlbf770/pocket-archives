...
type QuickView = "all" | "cards" | "prints" | "collections";
...
const sampleDescriptions: Record<string, string> = {
  "DEMO-001": "A sample listing for a 1997 Japanese Carddass card. The final listing will use photos and notes from the actual card.",
  "DEMO-002": "A sample listing for an early Japanese card featuring Ken Sugimori’s Haunter artwork.",
  "DEMO-003": "A sample listing for a small Japanese Pikachu promo from 1998.",
  "DEMO-004": "A sample listing for an early Japanese Pokémon print held in the Pocket Archives collection.",
  "DEMO-005": "A sample set bringing the three original starter lines together.",
};
...
export function ShopLanding() {
  ...
  const results = useMemo(() => demoInventory.filter((item) => {
    ...
    const printCategories = ["Ephemera", "Printed Matter"];
    const quickMatch = quickView === "all" ||
      (quickView === "cards" && cardCategories.includes(item.category)) ||
      (quickView === "prints" && printCategories.includes(item.category)) ||
